...
def plot_xt_trajectory_grid(x0_batch, x1_batch, q_positions, t_values, out_path):
    ...
    fig, axes = plt.subplots(
        n_rows,
        n_cols,
        figsize=(2.8 * n_cols, 2.75 * n_rows),
    )
    ...
    for r in range(n_rows):
        ...
        for c, (title, img) in enumerate(panels):
            ...
            ax.imshow(image_to_display(img), interpolation="bilinear")
    ...
    *,
    ...
    for r in range(n_rows):
        ...
        for c, t in enumerate(t_values):
            ...
            ax.imshow(image_to_display(xt), interpolation="bilinear")
            ax.imshow(field_q, cmap=cmap, alpha=alpha, vmin=0, vmax=1, interpolation="nearest")
            ax.scatter([qx], [qy], c="red", s=24)
...
def main():
    ...
    wiener_path = Path("data/models/wiener/celeba_hq_64")
    ...
    q_positions = [
        (int(0.41 * H), int(0.36 * W)),  # left eye
        (int(0.41 * H), int(0.64 * W)),  # right eye
        (int(0.53 * H), int(0.50 * W)),  # nose
        (int(0.28 * H), int(0.50 * W)),  # forehead
        (int(0.67 * H), int(0.50 * W)),  # mouth / chin
    ]

    t_values = [0.1, 0.3, 0.5, 0.7, 0.9]

    out_dir = Path("experiments_tom/figures/fm_masks_celeba_hq")
